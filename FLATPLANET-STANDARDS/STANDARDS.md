...
> Version: 2.5 | Last updated: 2026-04-06
...
"Please read the FlatPlanet standards. We are starting a new project called [name]. It does [description]. The tech stack is [stack]. Please set up the project structure following FlatPlanet standards, including CLAUDE.md, README.md, and CHANGELOG.md."
...
## 6. CLAUDE.md and CLAUDE-local.md

**`CLAUDE.md`** is the committed project brief — checked into the repo. Claude Code reads it first when opening any project.

**`CLAUDE-local.md`** is a local-only workspace file generated by the FlatPlanet HubApi. It is never committed to the repo (`CLAUDE-local.md` must be in `.gitignore`). It contains the live API token for the project plus all enforced tech stack standards for the project type. The frontend calls `GET /api/projects/{id}/claude-config/workspace` to generate and download it.
...
Every `CLAUDE.md` must contain:
  ...
  - DATA DICTIONARY: Live in Supabase — query via HubApi: GET /api/projects/{id}/schema/full (always read schema before naming tables or columns)
